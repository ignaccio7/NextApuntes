...
const restaurants: Restaurant[] = [
  ...
  {
    id: "12",
    name: "The Ice Cream Parlor Vector",
...
const api = {
  ...
  /**************** Con Google Sheets */
  // https://docs.google.com/spreadsheets/d/e/2PACX-1vT7pFUs7hbMa_aon0RkNkr_mMsGo9dZI5_8ehWptQhWr1HYb97jDwCxOhJ0zZpwXxR_X5xollAGH_mY/pub?output=csv
  listWithGoogleSheets: async (): Promise<Restaurant[]> => {
  ...
  fetchWithGoogleSheets: async (idSearch: string): Promise<Restaurant> => {
    ...
    const restaurant = restaurants.find((res) => {
      return res.split(",")[0] === idSearch;
    });

    if (!restaurant) throw new Error("404: Not found restaurant");

    const [id, name, description, address, score, ratings, image] = restaurant.split(",");
