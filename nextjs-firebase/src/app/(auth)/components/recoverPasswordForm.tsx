...
export default function RecoverPasswordForm() {
  ...
  const onSubmit = async (user: zod.infer<typeof loginFormSchema>) => {
    ...
    try {
      ...
      toast.success("Se ha enviado un correo a tu correo electronico")
      router.push("/")
    } catch (error: any) {
      // console.log(error);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-2 px-4 min-w-sm p-4">
      ...
      <div className="group-form text-sm text-muted-foreground text-center">
        <Link
          href={"/"}
        >
          {"🔙 Volver al inicio"}
